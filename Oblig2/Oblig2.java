/*-----------------------------------------------------------------------------
 * Brukernavn: syhd
 * UiO-INF2440: Oblig 2 (Vaar 2016)
 * Program to prime factorize big numbers both sequentially and in parallel
 *
 * Abbreviations:
...
class Oblig2 {
	public static void main(String[] args) {
		if(args.length != 2) {
			System.out.println("Usage: java Oblig2 <N> <#threads>");
			System.out.println("Enter 0 if you want #threads == #cores");
			return;
		}
		PFactorization pf = new PFactorization(Integer.parseInt(args[0]),
...
class PFactorization {
	...
	private void printReport() {
		...
		System.out.printf("EraSeq: %8.2fms, EraPar: %8.2fms ", 
		                  timeEraSeq, timeEraPar);
		System.out.printf("==> Speedup for Era: %.2f\n",timeEraSeq/timeEraPar);
		
		System.out.printf("FacSeq: %8.2fms, FacPar: %8.2fms ", 
	...
	/** Return a string containing a number and all of its prime factors */
	private String printFactors(long num, ArrayList<Long> factors) {
	...
	private void facSeq() {
		...
		for(int i = 100; i > 0 ; i--) {
			...
			while(p <= stop) {				
				...
				try {				
					p = nextPrime(p); 
				} catch(OutOfLimit exception) { break; }
			}
			if(cnum != 1) {
				//all primes <= N have been tried --> cnum itself must be prime
				factors.add((long) cnum);	
			}
			System.out.println("Seq: " + printFactors(num, factors));
		}
		time = System.nanoTime() - time;
	...
	private class NumMonitor {		
		...
		public String toString() {
			return printFactors(num, factors);
		}
	}
