/**----------------------------------------------------------------------------
 * Brukernavn: syhd
 * INF2440(UiO) - Spring 2016
 * Parallel Radix Sort (ascending order)
...
class Oblig3 {
	public static void main(String[] args) {
		...
		if(args.length == 1) {
			...
			System.out.println("USAGE: java Oblig3 <N> <#TESTS>");
			System.out.println("Leave the 2nd field empty if you " +
			                   "want it to be chosen automatically.");
			return;
		}
...
class MultiRadix {
	...
	/** Populate an array with random numbers generated based on a seed */
	private void populate(int[] a, long seed) {
	...
	public void testSort(int[] a, String message) {
		for(int i = 0; i < a.length-1; i++) {
			if(a[i] > a[i+1]) {
				System.out.println("("+message+") Wrong at index " + i + ": " +
				                   "a["+i+"]:"+a[i]+" > a["+(i+1)+"]:"+a[i+1]);
				return;
			}
		}
